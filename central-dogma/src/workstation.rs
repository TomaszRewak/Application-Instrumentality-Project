...
impl Workstation {
    ...
    async fn process(&self, message: proto::client_server::ClientToServerMessage) {
        match message.one_of {
            ...
            Some(proto::client_server::client_to_server_message::OneOf::CreateInstanceReply(
                create_instance_reply,
            )) => {
                self.process_create_instance_reply(create_instance_reply);
            }
    ...
    async fn process_login_request(&self, login_request: proto::client_server::LoginRequest) {
        ...
        self.sender.send(proto::client_server::ServerToClientMessage {
            one_of: Some(proto::client_server::server_to_client_message::OneOf::LoginReply(
                proto::client_server::LoginReply { error: None },
            )),
        }).await;
    }

    fn process_create_instance_reply(
        &self,
        start_application_reply: proto::CreateInstanceReply,
    ) {
    }

    fn process_run_task_reply(&self, run_task_reply: proto::RunTaskReply) {}

    fn process_heartbeat(&self, heartbeat: proto::client_server::Heartbeat)
    {
